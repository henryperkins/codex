...
> [!TIP]
> If you are familiar with the OpenAI-hosted Responses API, keep in mind that Azure adds **usage reporting headers**, a few **Azure-specific error codes**, and additional **security options** (Microsoft Entra ID / Managed Identity). These are covered in the following sections.


## Azure-specific usage headers

Every successful request to the Azure OpenAI service returns the standard JSON payload **and** usage-metadata headers that you can log for cost analysis or dashboards:

| Header | Purpose | Example |
| --- | --- | --- |
| `azure-openai-usage` | JSON string with `input_tokens`, `output_tokens`, `total_tokens`. Mirrors the `usage` object in the body but is immediately available from streaming or HEAD requests. | `{"input_tokens": 12, "output_tokens": 28, "total_tokens": 40}` |
| `x-ms-request-id` | Unique request identifier that can be provided to Microsoft Support when troubleshooting. | `b4519ae1-0b9a-4465-b56d-8f8eafb8560a` |
| `x-ms-model-id` | Resolved model deployment that actually processed the request. Handy when you route traffic to multiple deployments behind the same endpoint. | `gpt-4o-2024-08-06` |

Most client SDKs expose raw headers – if you are using the official `openai` Python package you can access them via `response.response.headers`.

```python
# python
completion = client.responses.create(...)
print(completion.response.headers["azure-openai-usage"])
```

When streaming, the headers are attached to the first SSE frame, so you can still capture costs even if the client aborts early.


## Error handling & throttling

Azure surfaces errors through the regular `status`/`error` body **and** the `x-ms-error-code` response header. Mapping this header lets you implement granular retry or user-facing messages.

| `x-ms-error-code` | HTTP | Meaning | Typical mitigation |
| --- | --- | --- | --- |
| `InvalidAuthenticationToken` | 401 | Bearer token missing / expired. | Refresh AAD token or verify API key. |
| `Unauthorized` | 401 | Provided key is invalid. | Check `AZURE_OPENAI_API_KEY`. |
| `Forbidden` | 403 | Caller lacks permissions on the OpenAI resource. | Assign `Cognitive Services OpenAI User` role. |
| `AzureOpenAI.RequestRateLimitExceeded` | 429 | Token or request rate exceeded the quota for this deployment. | Implement exponential back-off, retry after header. |
| `AzureOpenAI.QuotaExceeded` | 429 | Monthly subscription quota exhausted. | Increase quota or wait for reset. |
| `AzureOpenAI.DeploymentNotFound` | 404 | Deployment name not recognised. | Check `model` / `deployment` param. |

When you receive 429/5xx errors you should **always** inspect the `Retry-After` header (seconds) and delay the next attempt accordingly.


## Authentication & security flows

Azure supports **two** primary auth flows:

1. **API key** – simple header `api-key: <key>`. Keys can be regenerated in the Azure Portal and should be stored in **Azure Key Vault**. Ideal for server-side scripts and CI-agents.
2. **Microsoft Entra ID (OAuth 2.0 JWT)** – obtain a token for `https://cognitiveservices.azure.com/.default` using `DefaultAzureCredential` (managed identity, workload identity, interactive, etc.). Pass the token as the `Authorization: Bearer <token>` header.

Best practices:

- Prefer **Managed Identity** for Azure hosts (Functions, AKS, Container Apps) to remove secret rotation overhead.
- Restrict API keys with **role-based access control** (RBAC): assign only the `Cognitive Services OpenAI User` role to service principals that require run-only access.
- Always send the optional `x-ms-client-request-id` header (a GUID you generate) – it links your logs with Azure’s internal telemetry.
- For production, enable [Diagnostic Settings](https://learn.microsoft.com/azure/azure-monitor/essentials/diagnostic-settings) on the OpenAI resource to forward request logs to Log Analytics or Azure Sentinel.

> [!NOTE]
> When using Microsoft Entra ID you **do not** supply the `api-version`, `api-key`, or secret in the query string – only the Bearer token is required.
