...
impl ModelClient {
    ...
    pub fn with_advanced_features(mut self, features: AdvancedFeatures) -> Self {
    ...
    loop {
        ...
        match event.kind.as_str() {
            ...
            "response.in_progress" => {
                let event = ResponseEvent::InProgress;
                if tx_event.send(Ok(event)).await.is_err() {
                    return;
                }
            }
            "error" => {
                let event = ResponseEvent::Error(event.response.unwrap_or_default());
                if tx_event.send(Ok(event)).await.is_err() {
                    return;
                }
...
mod tests {
    ...
    fn test_advanced_features_integration() {
        ...
        assert!(!default_features.enable_background_processing);
        assert!(!default_features.enable_response_storage);
        assert!(default_features.enable_stream_resumption); // Only this is enabled by default

        // Test OpenAI-optimized features
        let openai_features = AdvancedFeatures::openai_optimized();
        assert!(openai_features.enable_response_chaining);
        assert!(openai_features.enable_response_storage);
        ...
        assert!(azure_features.enable_response_storage); // Required for background processing (store=true)

        // Test custom features
        let custom_features = AdvancedFeatures {
            enable_response_chaining: true,
            enable_background_processing: false,
            enable_response_storage: false,
            enable_stream_resumption: false,
            ..Default::default()
        };
        assert!(custom_features.enable_response_chaining);
        assert!(!custom_features.enable_background_processing);
        assert!(!custom_features.enable_response_storage);
        assert!(!custom_features.enable_stream_resumption);

        // Test auto-enabling of storage for background processing
        let mut incomplete_features = AdvancedFeatures {
            enable_background_processing: true,
            enable_response_storage: false, // This should auto-enable
            ..Default::default()
        };
        assert!(!incomplete_features.enable_response_storage); // Before validation
        incomplete_features.validate_and_fix();
        assert!(incomplete_features.enable_response_storage); // Auto-enabled by validation
    }
}
