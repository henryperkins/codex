...
const SEARCH_TOOL_DESCRIPTION_SNIPPETS: [&str; 3] = [
    "Tools of the apps (Calendar) are hidden until you search for them with this tool (`tool_search`).",
    "Matching tools are returned in the `tool_search_output` for the next model call.",
...
async fn search_tool_adds_discovery_instructions_to_tool_description() -> Result<()> {
    ...
    assert!(
        ...
        "tool_search description should include the expected workflow: {description:?}"
    );
    assert!(
